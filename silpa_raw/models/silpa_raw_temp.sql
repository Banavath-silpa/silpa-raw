...
source_data as (
	SELECT
		* 
		...
		from {{ source('dmn01_finsoi_bqd_silpa','tester_table') }} raw
),
	...
	SELECT 
		*,
        [
            ...
            IF( SAFE_CAST(coalesce(date_year,'01/01/1001') AS DATE FORMAT 'DD/MM/YYYY') is NOT NULL,
                STRUCT('NA' as col, 'NA' as error),STRUCT("date_year" as col,  CONCAT("dtypeCheckFailed [invalid date] [value: ",date_year,"]") as error)),
